...
export default function initStaking({ staking, apr, liquidity='ETH', lock, expiration_time }) {
    ...
    class Staking extends React.Component {
        ...
        handleClaimDivs = async (e) => {
            ...
            let path = [...new Set([rewardTokenAddress, WETH].map(a => a.toLowerCase()))]
        ...
        refreshBalance = async () => {
            ...
            try {
                ...
                let _tvlConstantDAI = reward_token_dai.balanceOf(staking._address) /* TVL of DAI on Staking */
                ...
                let [token_balance, pendingDivs, totalEarnedTokens, stakingTime,
                    ...
                    referralFeeEarned, total_stakers, tvlConstantDAI, tvlDYPS
                ] = await Promise.all([_bal, _pDivs, _tEarned, _stakingTime, _dTokens, _lClaimTime, _tvl,
                    _rFeeEarned, tStakers, _tvlConstantDAI, _tvlDYPS])
                ...
                let tvlUSD = new BigNumber(tvl).times(usd_per_lp).plus(usdValueDAI).toFixed(18)
                //console.log({tvlUSD})
        ...
        getApproxReturn = () => {
            ...
            let usd_per_eth = this.props.the_graph_result.token_data ? this.props.the_graph_result.usd_per_eth : 1

            return ( approxDeposit * usd_per_token * this.state.apy / usd_per_eth / 100 / 365 * approxDays)
        }
        ...
        render() {
            ...
            pendingDivs = new BigNumber(pendingDivs).div(10 ** TOKEN_DECIMALS).div(usd_per_bnb).toString(10)
                        ...
                        <div className="container">
                            <Popup show={this.state.popup} handleClose={this.hidePopup} >
                                <div className="earn-hero-content p4token-wrapper">
                                    ...
                                    <p>Stake your DYP tokens and earn 25% APR in AVAX. No Impermanent Loss.</p>
                                    <p>To start earning, all you need is to deposit DYP tokens into the
                                        Staking contract and earn AVAX as rewards.</p>
                                    <p>The staking pools have the REINVEST function integrated, meaning that
                                        ...
                                        you whenever your friends stake DYP. You do not need to stake, it
                                        will’ be automatically sent to you, free of gas fee.</p>
                                </div>
                                                    ...
                                                    <div className='form-group'>
                                                        ...
                                                        <p className='form-control  text-right' style={{ border: 'none', marginBottom: 0, paddingLeft: 0, background: 'transparent', color: '#222' }}><span style={{ fontSize: '1.2rem', color: 'rgb(255, 0, 122)' }}>{pendingDivsEth}</span> <small className='text-bold'>WETH</small></p>
                                                            ...
                                                            <div className='col-md-12'>
                                                                ...
                                                                <input value={Number(pendingDivs) > 0 ? `${pendingDivs} WAVAX` : `${pendingDivs} WAVAX`} onChange={e => this.setState({pendingDivs: Number(e.target.value) > 0 ? e.target.value : e.target.value})} className='form-control left-radius' placeholder='0' type='text' disabled />
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div className='form-row'>
